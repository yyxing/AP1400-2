...
// 将多个矩阵以指定轴拼接
    ...
    Matrix ero_swap(const Matrix &matrix, size_t r1, size_t r2) {
        ...
        if (r1 >= n1 || r2 >= n1) throw std::logic_error("row number must less than matrix rows");
        Matrix mat = zeros(n1, m1);
        for (int i = 0; i < n1; ++i) {
            for (int j = 0; j < m1; ++j) {
                int ii = i == r1 ? r2 : i == r2 ? r1 : i;
                mat[i][j] = matrix[ii][j];
            }
        }
        return mat;
    ...
    Matrix ero_multiply(const Matrix &matrix, size_t r, double c) {
        ...
        for (int i = 0; i < n1; ++i) {
            for (int j = 0; j < m1; ++j) {
                mat[i][j] = i == r ? matrix[i][j] * c : matrix[i][j];
            }
        }
        return mat;
    }
    ...
    Matrix ero_sum(const Matrix &matrix, size_t r1, double c, size_t r2) {
        check_empty({matrix});
        if (r1 >= matrix.size() || r2 >= matrix.size()) throw std::logic_error("row number must less than matrix rows");
        Matrix mat(matrix);
        auto& v1 = mat[r1];
        auto& v2 = mat[r2];
